...
export const SheetTrigger: React.FC<{ children: React.ReactNode; asChild?: boolean }> = ({ children, asChild }) => {
  ...
  if (asChild) {
    // FIX: Added `React.isValidElement` check to ensure children is a cloneable element, fixing the type error.
...
export const SheetClose: React.FC<{ children: React.ReactNode; asChild?: boolean }> = ({ children, asChild }) => {
  const { setIsOpen } = useSheet();
  if (asChild) {
    // FIX: Added `React.isValidElement` check to safely access child props and clone the element, fixing type errors.
    if (React.isValidElement(children)) {
      return React.cloneElement(children, {
